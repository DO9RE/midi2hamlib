...
if [[ -n "$SHELLCHECK" && -x $SHELLCHECK ]]; then
  ...
  for i in $(find ./ -type f | grep -v '\(/name$\)\|\(\.conf\)\|\(\.map\)\|\(\.txt$\)\|\(midi_monitor/\)\|\(\.git\)\|\(README\.MD\)\|\(/developer_communication/\)') ; do
    # SC2154: Disalb checkinf for referencing unassigned variable.
    ...
    # SC1090: disable follow sourced files.
    ...
    if ! $SHELLCHECK -s bash -e SC2154,SC1090,SC2164,SC2207 "$i" ; then
      EXITCODE=1
